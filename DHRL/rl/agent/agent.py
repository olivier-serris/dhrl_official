...
class DistReverseCritic(nn.Module):
    ...
    def get_dist(self, pi_inputs, actions):
        q_value = self._q_func(pi_inputs, actions)
        q_value = torch.clamp(q_value, min=1.0 / (self.gamma - 1.0) + 1)
        log_gamma = np.log(self.gamma)
        dist = torch.log(1.0 + q_value * (1.0 - self.gamma)) / log_gamma
        if self.args.q_offset:
            dist += 1.0
        return dist
...
class LowAgent(BaseAgent):
    ...
    def _process_inputs_actor(self, obs, goal):
        if self.args.absolute_goal:
            ...
            relative_goal = (
                goal[:, : self.env_params["sub_goal"]]
                - obs[:, : self.env_params["sub_goal"]]
            )
    ...
    def get_actions(self, obs, goal):
        ...
        with torch.no_grad():
            actions = self.actor(inputs).cpu().numpy()
    ...
    def _get_pairwise_dist(self, obs_tensor, ags_tensor):
        ...
        for obs_index in range(obs_tensor.shape[0]):
            ...
            obs, goal = self._preprocess_inputs(
                obs_repeat_tensor, goal_tensor[:, : self.args.subgoal_dim]
            )
            inputs = self._process_inputs_critic(obs, goal)
            with torch.no_grad():
                actions = self.to_tensor(
                    self.get_actions(
                        obs_repeat_tensor, goal_tensor[:, : self.args.subgoal_dim]
                    )
                )
        ...
        pairwise_dist = torch.stack(
            dist_matrix
        )  # pairwise_dist[i][j] is dist from i to j
        pairwise_dist = pairwise_dist.cpu().detach().numpy()
        return pairwise_dist

    def _get_dist_from_start(self, start, obs_tensor):
        start_repeat = np.ones(
            (obs_tensor.shape[0], np.squeeze(start).shape[0])
        ) * np.expand_dims(start, axis=0)
        obs, goal = self._preprocess_inputs(
            start_repeat, obs_tensor[:, : self.args.subgoal_dim]
        )
        inputs = self._process_inputs_critic(obs, goal)
        with torch.no_grad():
            actions = self.to_tensor(
                self.get_actions(start_repeat, obs_tensor[:, : self.args.subgoal_dim])
            )
            dist1 = self.critic1_g.get_dist(inputs, actions)
            dist2 = self.critic2_g.get_dist(inputs, actions)
            dist = torch.clamp(torch.minimum(dist1, dist2), min=0)
    ...
    def _get_dist_to_goal(self, obs_tensor, goal):
        goal_repeat = np.ones_like(
            obs_tensor[:, : self.args.subgoal_dim]
    ...
    def _get_point_to_point(self, point1, point2):
        ...
        with torch.no_grad():
            actions = torch.unsqueeze(
                self.to_tensor(
                    self.get_actions(point1, point2[: self.args.subgoal_dim])
                ),
                0,
            )
            dist1 = self.critic1_g.get_dist(inputs, actions)
    ...
    def target_update(self):
        net_utils.target_soft_update(
            source=self.actor, target=self.actor_targ, polyak=self.args.polyak
        )
        net_utils.target_soft_update(
            source=self.critic1, target=self.critic1_targ, polyak=self.args.polyak
        )
        net_utils.target_soft_update(
            source=self.critic2, target=self.critic2_targ, polyak=self.args.polyak
        )
        net_utils.target_soft_update(
            source=self.critic1_g, target=self.critic1_targ_g, polyak=self.args.polyak
        )
        net_utils.target_soft_update(
            source=self.critic2_g, target=self.critic2_targ_g, polyak=self.args.polyak
        )
    ...
    def state_dict(self):
        return {
            ...
            "critic1_g": self.critic1_g.state_dict(),
            "critic1_targ_g": self.critic1_targ_g.state_dict(),
            "critic2_g": self.critic2_g.state_dict(),
            "critic2_targ_g": self.critic2_targ_g.state_dict(),
        }
