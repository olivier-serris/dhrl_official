...
class Agent_DHRL(AgentAPI):
    def __init__(self, dhrl: "Algo") -> None:
        super().__init__()
        self.dhrl = dhrl

    def reset(self, agent_state, observation):
        self.dhrl.way_to_subgoal = 0
        return super().reset(agent_state, observation)

    def __call__(self, agent_state, observation):
        obs, dg = observation["observation"], observation["desired_goal"]
        action = self.dhrl.eval_get_actions(obs, dg)
...
class Algo(BaseAlgo):
    def __init__(
        ...
        self.evaluator = MazeEvaluator(
            test_env.unwrapped.spec.id, args.n_test_rollouts, vec_implem=False
        )
        config = vars(args)
    ...
    def get_actions(
        self, ob, bg, a_max=1, random_goal=False, act_randomly=False, graph=False
    ):
        # get subgoal
        if (self.curr_subgoal is None) or (self.way_to_subgoal == 0):
            ...
            if random_goal:
                sub_goal = np.random.uniform(
                    low=-1, high=1, size=self.env_params["sub_goal"]
                )
                ...
                if self.args.subgoal_noise_eps > 0.0:
                    ...
                    sub_goal_noise = self.args.subgoal_noise_eps * np.random.randn(
                        *sub_goal.shape
                    )
        ...
        if act_randomly:
            act = np.random.uniform(
                low=-a_max, high=a_max, size=self.env_params["l_action_dim"]
            )
        else:
    ...
    def low_agent_optimize(self):
        ...
        self.monitor.store(
            LowTimePerTrainIter=self.timer.get_time("low_train") / self.args.n_batches
        )
    ...
    def high_agent_optimize(self):
        ...
        for n_train in range(self.args.n_batches):
            batch = self.high_replay.sample(
                batch_size=self.args.batch_size, graphplanner=self.graphplanner
            )
        ...
        self.monitor.store(
            HighTimePerTrainIter=self.timer.get_time("high_train") / self.args.n_batches
        )

    def collect_experience(
        self, random_goal=False, act_randomly=False, train_agent=True, graph=False
    ):
        ...
        for timestep in range(self.env_params["max_timesteps"]):
            ...
            if (self.way_to_subgoal == 0) or (
                timestep == self.env_params["max_timesteps"] - 1
            ):
                high_ob_list.append(self.curr_highpolicy_obs.copy())
                high_ag_list.append(
                    self.curr_highpolicy_obs[: self.args.subgoal_dim].copy()
                )
                high_bg_list.append(bg.copy())
                high_a_list.append(self.curr_subgoal.copy())

            observation, _, _, _, info = self.env.step(act)
            ob = observation["observation"]
            ag = observation["achieved_goal"]
        ...
        low_experience = dict(
            ob=low_ob_list, ag=low_ag_list, bg=low_bg_list, a=low_a_list
        )
        high_experience = dict(
            ob=high_ob_list, ag=high_ag_list, bg=high_bg_list, a=high_a_list
        )
        ...
        if low_experience["ob"].ndim == 2:
            low_experience = {
                k: np.expand_dims(v, 0) for k, v in low_experience.items()
            }
        else:
            low_experience = {
                k: np.swapaxes(v, 0, 1) for k, v in low_experience.items()
            }
        if high_experience["ob"].ndim == 2:
            high_experience = {
                k: np.expand_dims(v, 0) for k, v in high_experience.items()
            }
        else:
            high_experience = {
                k: np.swapaxes(v, 0, 1) for k, v in high_experience.items()
            }
    ...
    def run(self):
        for n_init_rollout in range(self.args.n_initial_rollouts // self.num_envs):
            self.collect_experience(
                random_goal=True, act_randomly=True, train_agent=False, graph=False
            )
        ...
        for epoch in range(self.args.n_epochs):
            ...
            for n_iter in range(self.args.n_cycles):
                print(
                    "%d" % n_iter, end=" " if n_iter < self.args.n_cycles - 1 else "\n"
                )
                sys.stdout.flush()
            ...
            self.monitor.store(
                low_replay_fill_ratio=float(
                    self.low_replay.current_size / self.low_replay.size
                )
            )
            self.monitor.store(
                high_replay_fill_ratio=float(
                    self.high_replay.current_size / self.high_replay.size
                )
            )
            current_timestep = (
                self.args.n_initial_rollouts * self.env_params["max_timesteps"]
            ) + (epoch * self.env_params["max_timesteps"] * self.args.n_cycles)
            eval_infos = self.evaluator.evaluate(self.agent_api, None)
            wandb.log(eval_infos, step=current_timestep)
            print("eval at: ", current_timestep)

            # her_success = self.run_eval(
            #     epoch, use_test_env=True, render=self.args.eval_render
            # )
    ...
    def run_eval(self, epoch, use_test_env=False, render=False):
        ...
        for n_test in range(self.args.n_test_rollouts):
            ...
            TestEvn_Dist = np.linalg.norm(ag - bg)
